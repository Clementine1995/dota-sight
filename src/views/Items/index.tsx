...
function Items() {
  ...
  const itemSplit = itemsAll.reduce(
    (arr: ItemType[][], item) => {
      if (item.isBasic) {
        arr[0].push(item)
      } else {
  ...
  return (
    <ItemDispatch.Provider value={dispatch}>
      <div className="ds-items">
        <div className="ds-items__desc">{itemState.activeItem && <Detail activeItem={itemState.activeItem} />}</div>
        <div className="ds-items__container">
          {itemSplit.map((item, i) => (
            <ItemCol key={i} items={item} />
          ))}
        </div>
        <div className="ds-items__recipe">{itemState.activeItem && <Recipe activeItem={itemState.activeItem} />}</div>
      </div>
